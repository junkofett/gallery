...
create table roles(
  rol_id    bigserial   constraint pk_roles primary key,
  nom_rol   varchar(20) not null constraint uq_roles_nom_rol unique
);
...
create table usuarios(
  ...
  nom_rol           varchar(20)   constraint fk_usuarios_nom_rol
                                    references roles(nom_rol) on delete no action
                                    on update cascade default 'reg_user'
);
